...
int DFS(vector<uint>& arr, uint travel, uint now, int K){
    if(bitCount(now)==K || travel == 0){
        int ret=0;
        for(uint& i : arr){
            ret += (i & now) == i;
        }
    ...
    int p= ceil(log2(now+0.6));
    int mx = 0;
    while(travel>0){
        if(travel & 1<<p){
            travel &= ~(1<<p);
            mx = max(mx, DFS(arr, travel, now | 1<<p, K));
        }
        p++;
    }
    return mx;
